...
static const CGFloat CSToastHorizontalPadding   = 50.0;
...
- (void)showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)point {
    ...
    rotationAndPerspectiveTransform.m24 = 1.0 / -850;
    rotationAndPerspectiveTransform = CATransform3DTranslate(rotationAndPerspectiveTransform, 10.0f, 80.0f, 0.0f);
    layer.transform = rotationAndPerspectiveTransform;
    
    //[myView setFrame:CGRectMake(myView.frame.origin.x, myView.frame.origin.y, myView.frame.size.width, myView.frame.size.height)];
...
//    
//    toast.transform = CGAffineTransformMakeRotation(-M_1_PI / 2);
//    [UIView animateWithDuration:0.3f
//                          delay:0
//                        options:UIViewAnimationOptionCurveEaseInOut
//                     animations:^(void)
//     {
//         toast.transform = CGAffineTransformMakeRotation(0);
//         CGRect step1Rect = toast.frame;
//         step1Rect.origin.y = (self.frame.size.height/2)-(toast.frame.size.height/2);
//         toast.frame = step1Rect;
//         toast.alpha = 1.0;
//     }
//                     completion:^(BOOL finished) {
//                         if(finished){
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    if (CSToastDisplayShadow) {
        wrapperView.layer.shadowColor = [[UIColor colorWithRed:0/255.0f green:0/255.0f blue:0/255.0f alpha:0.0f] CGColor];
...
//    wrapperView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:CSToastOpacity];
    wrapperView.backgroundColor = [UIColor colorWithRed:0/255.0f green:0/255.0f blue:0/255.0f alpha:0.8f] ;
...
//        CGSize expectedSizeMessage = [self sizeForString:message font:messageLabel.font constrainedToSize:maxSizeMessage lineBreakMode:messageLabel.lineBreakMode];
    ...
    if(messageLabel != nil) {
        messageWidth = messageLabel.bounds.size.width-20    ;
        messageHeight = messageLabel.bounds.size.height;
        messageLeft = imageLeft + imageWidth + CSToastHorizontalPadding-20;
...
//    CGFloat wrapperWidth = MAX((imageWidth + (CSToastHorizontalPadding * 2)), (longerLeft + longerWidth + CSToastHorizontalPadding));    
    ...
    wrapperView.frame = CGRectMake(20.0, 0.0, 280, 60);
    
    wrapperView.center=CGPointMake(wrapperView.frame.size.width/2, (wrapperView.frame.origin.y+wrapperHeight)/2);
    if(titleLabel != nil) {
    ...
    if(messageLabel != nil) {
        messageLabel.frame = CGRectMake(messageLeft, 10, messageWidth, messageHeight);
