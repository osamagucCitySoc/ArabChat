...
@property (weak, nonatomic) IBOutlet NZCircularImageView *profileImage;
...
+ (void)showWithNotificationMessage:(NSString *)message photo:(NSString*)photo
{
    [AGPushNoteView showWithNotificationMessage:message photo:photo completion:^{
        //Nothing.
    }];
}

+ (void)showWithNotificationMessage:(NSString *)message photo:(NSString*)photo completion:(void (^)(void))completion {
    ...
    if (message) {
        ...
        [PUSH_VIEW.profileImage setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",@"http://moh2013.com/arabDevs/arabchat/images/",photo]] placeholderImage:[UIImage imageNamed:@"loading.png"] usingActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite];
